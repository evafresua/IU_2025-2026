class alumnograduacion extends Validations{
    ...
    // columnas a mostrar en la tabla y atributos con formato especial
    this.columnasamostrar = ['alumnograduacion_login','alumnograduacion_nombre','alumnograduacion_apellidos'];
    this.mostrarespecial = ['alumnograduacion_fotoacto'];
        ...
        if (esTest == 'test'){
            ...
            try{
                // follow the same UI wiring pattern used by other entity classes
                document.getElementById('text_title_page').classList.add('text_titulo_page_'+this.nombreentidad);
                document.getElementById('text_title_page').setAttribute('onclick','entidad = new alumnograduacion();');
                if (this.dom) this.dom.show_element('IU_manage_entity', 'block');
                // render the form into the container used by the app
                document.getElementById('contenedor_IU_form').innerHTML = this.manual_form_creation();
                if (this.dom) this.dom.show_element('Div_IU_form','block');
                // place field validations and icon-button for ADD action
                if (this.dom && typeof this.dom.colocarvalidaciones === 'function') this.dom.colocarvalidaciones('form_iu','ADD');
                if (this.dom && typeof this.dom.colocarboton === 'function') this.dom.colocarboton('ADD');
            }catch(e){}
        }
    }   
    ...
    manual_form_creation(){
        var form_content = `
            <form id='form_iu' action="javascript:entidad.ADD();" method="POST" enctype="multipart/form-data" onsubmit="return entidad.ADD_submit_alumnograduacion();">
...
// Validación del campo de texto que contiene el nombre del fichero ya existente
ADD_alumnograduacion_fotoacto_validation(){
    var el = document.getElementById('alumnograduacion_fotoacto');
    if (!el) return true;
    var v = el.value || '';
    if (v === ''){ this.dom.mostrar_exito_campo('alumnograduacion_fotoacto'); return true; }
    if (!(this.max_size('alumnograduacion_fotoacto',40))){ this.dom.mostrar_error_campo('alumnograduacion_fotoacto','alumnograduacion_fotoacto_max_size_KO'); return 'alumnograduacion_fotoacto_max_size_KO'; }
    if (!(this.format('alumnograduacion_fotoacto','^[a-zA-Z0-9._\/-]+$'))){ this.dom.mostrar_error_campo('alumnograduacion_fotoacto','alumnograduacion_fotoacto_format_KO'); return 'alumnograduacion_fotoacto_format_KO'; }
    this.dom.mostrar_exito_campo('alumnograduacion_fotoacto');
    return true;
}
...
// Wrappers personalize_ para las validaciones de fichero/atributo
personalize_alumnograduacion_fotoacto_ADD(){ return this.ADD_alumnograduacion_fotoacto_validation(); }
personalize_alumnograduacion_fotoacto_EDIT(){ return this.EDIT_alumnograduacion_fotoacto_validation(); }
...
personalize_nie_format(){
    ...
    // Formatea atributos para presentación si están en mostrarespecial
        ...
        try{
            switch(atributo){
                case 'alumnograduacion_fotoacto':
                    ...
                    return valorentrada + `  <a class="link_alumnograduacion_fotoacto" href="http://193.147.87.202/ET2/filesuploaded/files_alumnograduacion_fotoacto/`+valorentrada+`"><img src="./iconos/FILE.png" /></a>`;
                default:
                    return valorentrada;
            }
    ...
    // Wrappers personalize_ delegando en las validaciones existentes
    personalize_alumnograduacion_dni_ADD(){ return this.ADD_alumnograduacion_dni_validation(); }
    personalize_alumnograduacion_dni_EDIT(){ return this.EDIT_alumnograduacion_dni_validation(); }
    personalize_nuevo_alumnograduacion_fotoacto_ADD(){ return this.ADD_nuevo_alumnograduacion_fotoacto_validation(); }
    personalize_nuevo_alumnograduacion_fotoacto_EDIT(){ return this.EDIT_nuevo_alumnograduacion_fotoacto_validation(); }
